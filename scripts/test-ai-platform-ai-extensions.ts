...
assert.doesNotThrow(()=>assertAiExtensionTransition('active','applying'));
...
assert.match(helper,/CHANNEL\(linkedid\)/);
assert.match(helper,/MixMonitor/);
assert.match(helper,/__REC_STATUS=RECORDING/);
assert.match(helper,/__MIXMON_ID/);
assert.match(helper,/CDR\(recordingfile\)/);
...
assert.equal(inspection.plannedDialplan.continuousRecording,true);
assert.equal(inspection.plannedDialplan.recordingStartsBeforeStasis,true);
assert.equal(inspection.plannedDialplan.recordingPersistsAcrossHandoff,true);
assert.equal(inspection.plannedDialplan.recordingFilenamePattern,'ai-<linkedid>.wav');
const managedBlock=helper.slice(helper.indexOf('function customBlock'),helper.indexOf('$command ='));
assert.equal((managedBlock.match(/MixMonitor/g)||[]).length,1,'managed dialplan contains one continuous recorder');
assert.ok(managedBlock.indexOf('MixMonitor')<managedBlock.indexOf('Stasis'), 'recording starts before Stasis');
assert.doesNotMatch(managedBlock,/StopMixMonitor/, 'handoff must not stop the caller recording');
...
assert.match(service,/activeExtensionPreservedDuringPreview/);
assert.match(service,/status IN\('active','preview_ready','disabled','conflict','sync_failed'\)/);
assert.match(service,/String\(reusable\.status\)!=="active"/);
