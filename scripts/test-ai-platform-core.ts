...
const receptionistConfig=buildReceptionistLiveConfig({templateId:11,behaviorProfileId:3,transferPolicyId:1,autonomyPolicyId:1,toolIds:[7],actionDefinitionId:1,permissionKeys:[]});const receptionistValidation=await new AgentConfigurationValidator(store).validate(tenant.id,{templateId:11,behaviorProfileId:3,config:receptionistConfig,prompt:RECEPTIONIST_SYSTEM_PROMPT});assert.deepEqual(receptionistValidation,{valid:true,errors:[]});assert.equal(receptionistConfig.voiceEnabled,true);assert.equal(receptionistConfig.voice.provider,'synthetic');assert.equal(receptionistConfig.voice.mode,'speech_to_speech');assert.equal(receptionistConfig.voice.interruptible,true);assert.equal(receptionistConfig.voice.maxSentences,2);assert.equal(receptionistConfig.voice.maxWords,30);assert.equal(receptionistConfig.voice.softMaxWords,20);assert.deepEqual(receptionistConfig.voice.targetWords,[8,18]);assert.equal(receptionistConfig.voice.maxGeneratedUnits,320);assert.equal(receptionistConfig.voice.retryGeneratedUnits,512);assert.equal(receptionistConfig.voice.greetingGeneratedUnits,192);assert.equal(receptionistConfig.voice.delayedPlayoutStartupMs,500);assert.deepEqual(receptionistConfig.voice.typicalResponseSeconds,[2,5]);assert.equal(receptionistConfig.voice.softResponseSeconds,6);assert.equal(receptionistConfig.voice.maxResponseAudioSeconds,12);assert.equal(receptionistConfig.voice.queuedAudioWarningSeconds,6);assert.equal(receptionistConfig.voice.queuedAudioHardLimitSeconds,12);assert.equal(receptionistConfig.voice.maxPendingResponses,0);assert.equal(receptionistConfig.humanTransferPriority,'highest');
...
for(const table of ['ai_agent_templates','ai_agent_prompt_versions','ai_transfer_policies','ai_autonomy_policies','ai_agent_test_sessions','ai_knowledge_sources','ai_knowledge_versions','ai_knowledge_documents','ai_knowledge_chunks','ai_agent_knowledge','ai_training_items','ai_training_versions','ai_conversations','ai_conversation_messages','ai_tool_executions'])assert.ok(migration.includes(`CREATE TABLE IF NOT EXISTS ${table}`));
...
for(const permission of ['view_ai_voice_agents','manage_ai_voice_agents','test_ai_voice_agents','manage_ai_voice_routes','view_ai_voice_transcripts','export_ai_voice_transcripts'])assert.ok(migration.includes(`'${permission}'`));
for(const table of ['ai_voice_transcript_utterances','ai_voice_call_insights'])assert.ok(migration.includes(`CREATE TABLE IF NOT EXISTS ${table}`));
