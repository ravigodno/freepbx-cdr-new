...
assert.match(setupGuide,/pbxpuls-bitrix-connector-v5\.8\.4\.zip/);
...
const reportsTab=fs.readFileSync('src/components/reports/ReportsTab.tsx','utf8'),leadsDashboard=fs.readFileSync('src/components/reports/dashboard/SiteFormLeadsDashboard.tsx','utf8');
assert.match(reportsTab,/site-leads.*Заявки/);assert.match(reportsTab,/SiteFormLeadsDashboard/);
for(const text of ['Динамика заявок','Воронка обработки','Статусы заявок','Источники','Сайты','Формы'])assert.ok(leadsDashboard.includes(text));
for(const group of ['day','source','form','site','status'])assert.match(leadsDashboard,new RegExp(`breakdown\\('${group}'\\)`));
