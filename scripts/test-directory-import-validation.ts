...
assert.ok(frontend.includes('Источник данных'));assert.ok(frontend.includes('Перетащите CSV, TXT или Excel сюда'));assert.doesNotMatch(frontend,/isImportOpen|aria-label="Загрузка файла"/);assert.ok(frontend.includes('Ошибки по строкам и столбцам'));assert.ok(frontend.includes('max-h-[calc(100vh-330px)]'));
