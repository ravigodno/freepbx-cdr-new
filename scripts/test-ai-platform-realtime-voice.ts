...
async function run() {
  ...
  events.length = 0; await adapter.startInitialGreeting('Здравствуйте. Чем могу помочь?'); assert.equal(events.filter(event => event.type === 'response_started').length, 1); assert.equal(events.filter(event => event.type === 'output_audio').length, 3); assert(events.some(event => event.type === 'transcript' && event.text === 'Здравствуйте. Чем могу помочь?'));
  ...
  assert.match(router, /Raw audio payload is forbidden/); assert.doesNotMatch(router, /apiKey|Authorization|providerSessionIdHash/); assert.match(migration, /ai\.realtime_voice_enabled','false/); assert.match(migration, /ai\.realtime_voice_provider','synthetic/); assert.match(service, /transferRequired=true/); assert.match(service, /toolCalls>2/);assert.match(service,/greetingStatus!=='not_started'/); assert.doesNotMatch(service, /asterisk\s+-rx|external_host|createBridge|answerChannel/i);
