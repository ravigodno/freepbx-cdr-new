...
export default function McnTelecomBalancePanel({token,canManage,canSync,mode,refreshKey}:Props){
  ...
  if(mode==='summary')return <section className="grid gap-4 border-t border-slate-200 px-4 py-3 sm:grid-cols-2 lg:grid-cols-[minmax(260px,1.4fr)_minmax(170px,1fr)_minmax(180px,0.8fr)_minmax(175px,auto)] lg:items-center dark:border-slate-700">
    ...
    <div className="min-w-[180px]"><div className="text-[10px] uppercase text-slate-400">Текущий баланс</div><div className="mt-1 whitespace-nowrap font-mono text-lg font-black" title="Баланс без кредитного лимита">{money(data?.balance,data?.currency||'RUB')}</div>
      <div className="mt-1 text-[10px] text-slate-500">Кредитный лимит: {money(data?.creditLimit,data?.currency||'RUB')}</div></div>
    <div className="min-w-[175px] border-slate-200 text-[10px] text-slate-500 sm:text-right lg:border-l lg:pl-4 dark:border-slate-700">Обновлено:<br/><span className="whitespace-nowrap font-medium text-slate-700 dark:text-slate-300">{data?.measuredAt?new Date(data.measuredAt).toLocaleString('ru-RU'):'Нет данных'}</span>
    ...
    {data?.safeErrorCode&&<div className="text-[11px] text-red-600 lg:col-span-4">{data.safeErrorCode}{data.measuredAt&&' · Показан последний успешный снимок'}</div>}
    {error&&<div role="alert" className="text-[11px] text-red-600 lg:col-span-4">{error}</div>}{notice&&<div role="status" className="text-[11px] text-slate-500 lg:col-span-4">{notice}</div>}
  </section>;
