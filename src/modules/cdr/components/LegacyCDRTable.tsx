...
import CDRTimeCell, { formatCdrDateTime } from './CDRTimeCell';
import CDRRegistryIcon from './CDRRegistryIcon';
        ...
        {registryRows.map((entry, index) => {
          if (entry.kind === 'lead') {
            ...
            return <tr key={`site-form-${lead.id}`} className="bg-blue-50/70 transition-colors hover:bg-blue-100/70 dark:bg-blue-950/15">
              <td className="py-4 px-4 font-normal text-slate-705 dark:text-slate-350"><div className="flex items-center gap-3"><CDRRegistryIcon kind="site-form" /><div className="flex flex-col"><span className="font-bold text-slate-800 dark:text-slate-200 text-[13px] tracking-tight">{formatCdrDateTime(String(lead.created_at || ''), cdrDateTimeFormat, cdrShowSeconds, cdrUseBrowserTimezone, cdrHourCycle)}</span><span className="text-[11px] text-slate-400 font-mono mt-0.5">ID: FORM-{lead.id}</span></div></div></td>
