...
export default function AiAgentBuilderPage(props:Props){
 ...
 const permissions=useMemo(()=>({...props,canEdit:props.canCreate,canPublishAgents:props.canPublishAgents,canManageKnowledge:props.hasPermission('manage_ai_knowledge'),canPublishKnowledge:props.hasPermission('publish_ai_knowledge')}),[props]);
 return <section className="w-full space-y-4">
  ...
  <main className="min-w-0">
   ...
   {section==='skills'&&(props.canViewSkills??props.hasPermission('view_ai_skills'))&&<SimpleSurface title="Навыки" text="Настройте, что AI-сотрудники умеют делать."><SkillBuilderPanel token={token} enabled canManage={props.canManageSkills??props.hasPermission('manage_ai_skills')} canPublish={props.canPublishSkills??props.hasPermission('publish_ai_skills')}/></SimpleSurface>}
   {section==='knowledge'&&props.canViewPlatform&&<SimpleSurface title="Базы знаний" text="Подключите инструкции, ответы и справочники."><AgentKnowledgeTrainingPage token={token} enabled canViewKnowledge={props.canViewKnowledge} canViewTraining={props.canViewTraining} canManageKnowledge={props.hasPermission('manage_ai_knowledge')} canPublishKnowledge={props.hasPermission('publish_ai_knowledge')}/></SimpleSurface>}
...
function SettingsWorkspace(props:Props&{expertMode:boolean;onExpertModeChange:(value:boolean)=>void}){
 const[sub,setSub]=useState<'voices'|'providers'|'integrations'|'access'|'diagnostics'>(()=>location.pathname.startsWith('/ai-platform/diagnostics')?'diagnostics':'providers');
 const tabs=[['providers','Провайдеры',Activity],['voices','Голоса',Volume2],['integrations','Интеграции',Plug],['access','Права доступа',KeyRound],...(props.canExpertMode?[['diagnostics','Диагностика',Settings2] as const]:[]) ] as const;
 return <section className="space-y-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-black">Настройки</h1><p className="text-sm text-slate-500">Голоса, подключения и права доступа.</p></div>{props.canExpertMode&&<label className="flex items-center gap-2 rounded-lg border bg-white px-3 py-2 text-sm font-bold"><input type="checkbox" checked={props.expertMode} onChange={event=>props.onExpertModeChange(event.target.checked)}/>Показать расширенные настройки</label>}</div><div className="flex flex-wrap gap-2">{tabs.map(([key,label,Icon])=><button key={key} onClick={()=>setSub(key)} className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-bold ${sub===key?'bg-blue-600 text-white':'border bg-white'}`}><Icon className="h-4 w-4"/>{label}</button>)}</div>
  {sub==='voices'&&<VoiceSettingsPanel token={props.token} agentId={1} canManage={props.canManageVoiceSettings} expertMode={props.expertMode} canManageRuntimePrompts={['su','admin'].includes(String(props.session?.role||'').toLowerCase())}/>}
  {sub==='providers'&&<ProviderSettingsPanel token={props.token} canManage={props.canManageProviders}/>}
