...
export default function NotificationCenterSettings({
  ...
  const request = useCallback(
    async (path: string, options: RequestInit = {}) => {
      ...
      if (!response.ok)
        throw new Error(
          body.safeMessage || body.error || "Операция недоступна",
        );
      return body;
    },
    [headers],
  ...
  const loadLog = useCallback(async () => {
    ...
    const body = await request(`/api/notifications/deliveries?${query}`);
    setRows(body.rows || []);
  }, [canViewLog, filters, request]);
  useEffect(() => {
    void load();
  }, [load]);
  useEffect(() => {
    if (data) void loadLog();
  }, [data, loadLog]);
  ...
  const action = async (kind: "test" | "preview" | "clear") => {
    setBusy(kind);
    setMessage(null);
    try {
      if (kind === "test") {
        await request("/api/notifications/test", { method: "POST" });
        setMessage({ text: "Тестовое сообщение доставлено" });
      } else if (kind === "preview") {
        const body = await request("/api/notifications/preview");
        setPreview(body.candidates || {});
        setMessage({ text: "Предпросмотр выполнен без отправки" });
      } else {
        const body = await request("/api/notifications/telegram/token", {
          method: "DELETE",
        });
  ...
  const openBotFather = () => {
    ...
    if (navigator.clipboard?.writeText)
      void navigator.clipboard
        ...
        .then(() =>
          setMessage({
            text: "Команда /newbot скопирована — вставьте её в BotFather и нажмите Enter",
          }),
        )
        .catch(() =>
          setMessage({ text: "В BotFather выберите команду /newbot" }),
        );
    else setMessage({ text: "В BotFather выберите команду /newbot" });
  };
  const telegramSetup = async (kind: "check" | "discover") => {
    setBusy(kind);
    setMessage(null);
    try {
      if (kind === "check") {
        const body = await request("/api/notifications/telegram/check", {
          method: "POST",
        });
        setBot(body.bot);
        setMessage({ text: `Бот @${body.bot.username} доступен` });
      } else {
        const body = await request(
          "/api/notifications/telegram/discover-chats",
          { method: "POST" },
        );
        setTelegramChats(body.chats || []);
        setMessage({
          text: body.chats?.length
            ? "Доступные чаты найдены"
            : "Чаты не найдены: добавьте бота и отправьте сообщение в группу или канал",
        });
      }
    } catch (error: any) {
      setMessage({ error: true, text: error.message });
  ...
  if (!data)
    return (
      <div className="rounded-xl bg-rose-50 p-4 text-sm text-rose-700">
        {message?.text || "Центр уведомлений недоступен"}
      </div>
    );
  const grouped = Object.keys(CATEGORY_LABELS).map((category) => ({
    category,
    items: rules.filter((rule: any) => rule.category === category),
  }));
  return (
    <div className="space-y-5">
      <section className="rounded-2xl border bg-slate-50 p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          ...
          <label className="text-xs font-bold">
            ...
            <input
              disabled={!canManage}
              value={global.objectName || ""}
              onChange={(e) => updateGlobal("objectName", e.target.value)}
          ...
          <label className="text-xs font-bold">
            ...
            <select
              disabled={!canManage}
              value={global.minimumSeverity || "warning"}
              onChange={(e) => updateGlobal("minimumSeverity", e.target.value)}
              className="mt-1 w-full rounded-lg border bg-white px-3 py-2"
            >
          ...
          <label className="flex items-center gap-2 text-xs font-bold">
            <input
              disabled={!canManage}
              type="checkbox"
              checked={global.notifyOnRecovery !== false}
              onChange={(e) =>
                updateGlobal("notifyOnRecovery", e.target.checked)
              }
            />
            Уведомлять о восстановлении
          </label>
          <label className="text-xs font-bold">
            Стандартный cooldown, секунд
            <input
              disabled={!canManage}
              type="number"
              min="0"
              value={global.defaultCooldownSeconds || 3600}
              onChange={(e) =>
                updateGlobal("defaultCooldownSeconds", Number(e.target.value))
              }
              className="mt-1 w-full rounded-lg border bg-white px-3 py-2"
            />
          </label>
      ...
      <section className="rounded-2xl border p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <div className="text-xs text-slate-500">
            Последняя доставка: {channel.lastSuccessAt || "—"}
            <br />
            Последняя ошибка:{" "}
            {channel.lastError
              ? STATUS_LABELS[channel.lastError] || channel.lastError
              : "—"}
          </div>
          <label className="text-xs font-bold">
            Bot Token
            <input
              ...
              placeholder={
                channel.hasToken
                  ? "Токен сохранён · оставьте пустым без изменения"
                  : "Telegram Bot Token"
              }
          ...
          {channel.hasToken && (
            <label className="flex items-center gap-2 text-xs">
              ...
              />
              Разрешить замену сохранённого токена
            </label>
          )}
        </div>
        <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50 p-4">
          <b className="text-xs">Мастер Telegram</b>
          <p className="mt-1 text-[11px] text-slate-600">
            Если бота ещё нет, кнопка откроет официальный BotFather и скопирует
            команду <code>/newbot</code>. Вставьте её, нажмите Enter, затем
            укажите название и уникальный username с окончанием <code>bot</code>
            . Полученный токен вставьте выше и сохраните.
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <button
              type="button"
              disabled={!canManage || !channel.hasToken || !!busy}
              onClick={() => void telegramSetup("check")}
              className="rounded-lg border bg-white px-3 py-2 text-xs font-bold"
            >
              Проверить бота
            </button>
            {bot?.username && (
              <>
                <a
                  href={`https://t.me/${bot.username}?startgroup&admin=manage_chat`}
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white"
                >
                  Создать / выбрать группу
                </a>
                <a
                  href={`https://t.me/${bot.username}?startchannel&admin=post_messages`}
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white"
                >
                  Создать / выбрать канал
                </a>
              </>
            )}
            <button
              type="button"
              disabled={!canManage || !channel.hasToken || !!busy}
              onClick={() => void telegramSetup("discover")}
              className="rounded-lg border bg-white px-3 py-2 text-xs font-bold"
            >
              Найти Chat ID
            </button>
            ...
            <div className="mt-3 space-y-2">
              {telegramChats.map((chat) => (
                <button
                  type="button"
                  key={chat.chatId}
                  onClick={() => updateChannel("chatId", chat.chatId)}
                  className="flex w-full items-center justify-between rounded-lg border bg-white px-3 py-2 text-left text-xs"
                >
                  <span>
                    <b>{chat.title}</b>
                    <small className="ml-2 text-slate-400">{chat.type}</small>
                  </span>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {channel.hasToken && (
            <button
              ...
              disabled={!canManage || !!busy}
              onClick={() =>
                window.confirm("Удалить сохранённый Telegram Bot Token?") &&
                void action("clear")
              }
              className="flex items-center gap-2 rounded-lg border border-rose-200 px-3 py-2 text-xs font-bold text-rose-700"
            >
      ...
      <section className="rounded-2xl border p-5">
        <h4 className="text-sm font-black">События</h4>
        <div className="mt-4 space-y-3">
          {grouped.map((group) => {
            const categoryEnabled =
              global.categories?.[group.category] !== false;
            return (
              <div key={group.category} className="rounded-xl border">
                <label className="flex items-center gap-2 border-b bg-slate-50 p-3 text-xs font-black">
                  <input
                    disabled={!canManage}
                    type="checkbox"
                    checked={categoryEnabled}
                    onChange={(e) =>
                      updateGlobal("categories", {
                        ...(global.categories || {}),
                        [group.category]: e.target.checked,
                      })
                    }
                  />
                ...
                <div
                  className={`space-y-2 p-3 ${categoryEnabled ? "" : "opacity-45"}`}
                >
                  {group.items.map((rule: any) => {
                    const definition = catalog.find(
                      (item: any) => item.eventType === rule.eventType,
                    );
                    return (
                      <div
                        key={rule.eventType}
                        className="grid gap-2 rounded-lg border p-3 md:grid-cols-[minmax(220px,1fr)_120px_140px_170px]"
                      >
                        <label className="flex items-start gap-2 text-xs">
                          <input
                            disabled={!canManage || !categoryEnabled}
                            type="checkbox"
                          ...
                          />
                          <span>
                            <b>{definition?.title || rule.eventType}</b>
                            <small className="block text-slate-400">
                              {rule.eventType}
                              {definition?.producer === "registered"
                                ? " · источник пока не подключён"
                                : ""}
                            </small>
                          </span>
                        </label>
                        ...
                        <label className="flex items-center gap-2 text-[10px]">
                          <input
                            disabled={!canManage || !categoryEnabled}
                            type="checkbox"
                            checked={rule.notifyOnRecovery === true}
                            onChange={(e) =>
                              updateRule(rule.eventType, {
                                notifyOnRecovery: e.target.checked,
                              })
                            }
                          />
                          Recovery
                        ...
                        <div className="text-[10px]">
                          {rule.eventType === "balance.low" && (
                            <label>
                              Порог
                              <input
                                disabled={!canManage || !categoryEnabled}
                                type="number"
                                value={rule.parameters?.threshold ?? 1500}
                                onChange={(e) =>
                                  updateRule(rule.eventType, {
                                    parameters: {
                                      ...rule.parameters,
                          ...
                          {rule.eventType === "calls.missed_unreturned" && (
                            <span className="block rounded border bg-slate-50 px-2 py-1">
                              Общий SLA:{" "}
                              {data.callQuality?.missedCallSlaMinutes ?? "—"}{" "}
                              мин.
                            </span>
                          )}
                          {rule.eventType === "trunk.down" && (
                            <label>
                              Ошибок подряд
                              <input
                                disabled={!canManage || !categoryEnabled}
                                ...
                                min="1"
                                value={
                                  rule.parameters?.consecutiveFailures ?? 2
                                }
                                onChange={(e) =>
                                  updateRule(rule.eventType, {
                                    parameters: {
                          ...
                          {rule.eventType === "gsm.balance_low" && (
                            <label>
                              Порог, RUB
                              <input
                                disabled={!canManage || !categoryEnabled}
                                type="number"
                                min="0"
                                value={rule.parameters?.threshold ?? 100}
                                onChange={(e) =>
                                  updateRule(rule.eventType, {
                                    parameters: {
                                      ...rule.parameters,
                          ...
                          {rule.eventType === "gsm.sip_latency_high" && (
                            <label>
                              Порог, мс
                              <input
                                disabled={!canManage || !categoryEnabled}
                                type="number"
                                min="1"
                                value={rule.parameters?.thresholdMs ?? 57}
                                onChange={(e) => updateRule(rule.eventType, { parameters: { ...rule.parameters, thresholdMs: Number(e.target.value) } })}
                                className="mt-1 w-full rounded border px-2 py-1"
                              />
                            </label>
                          )}
                          {rule.eventType === "gsm.signal_low" && (
                            <label>
                              Порог CSQ
                              <input
                                disabled={!canManage || !categoryEnabled}
                                ...
                                min="0"
                                max="31"
                                value={rule.parameters?.thresholdCsq ?? 10}
                                onChange={(e) => updateRule(rule.eventType, { parameters: { ...rule.parameters, thresholdCsq: Number(e.target.value) } })}
                                className="mt-1 w-full rounded border px-2 py-1"
                              />
                            </label>
                          )}
                          {["gsm.sip_latency_high", "gsm.signal_low", "gsm.port_unavailable"].includes(rule.eventType) && (
                            <label className="mt-1 block">
                              ...
                              <input
                                ...
                                type="number"
                                min="1"
                                value={rule.parameters?.consecutiveFailures ?? 2}
                                onChange={(e) => updateRule(rule.eventType, { parameters: { ...rule.parameters, consecutiveFailures: Number(e.target.value) } })}
                                className="mt-1 w-full rounded border px-2 py-1"
                              />
                            </label>
                          )}
      ...
      {preview && (
        <div className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-xs">
          Найдено без отправки: баланс — {preview.balance?.length || 0},
          пропущенные — {preview.missedCalls?.length || 0}, транки —{" "}
          {preview.trunks?.length || 0}, система — {preview.system?.length || 0}
          , GSM — {preview.gsm?.length || 0}.
        </div>
      )}
      {message && (
        <div
          className={`flex gap-2 rounded-xl p-3 text-xs font-bold ${message.error ? "bg-rose-50 text-rose-700" : "bg-emerald-50 text-emerald-700"}`}
        >
      ...
      {canManage && (
        <div className="flex justify-end">
          <button
            ...
            disabled={!!busy}
            onClick={() => void save()}
            className="rounded-lg bg-blue-600 px-5 py-2 text-xs font-black text-white"
          >
      ...
      {canViewLog && (
        <section className="rounded-2xl border p-5">
          <h4 className="text-sm font-black">Журнал доставок</h4>
          <div className="my-3 grid gap-2 md:grid-cols-5">
            <input
              type="date"
              value={filters.from}
