...
import { CheckCircle2, ChevronDown, Copy, KeyRound, Radio, Server, X } from "lucide-react";
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-blue-200 bg-blue-50/50 shadow-sm">
      ...
      {expanded && <div className="border-t border-blue-200 bg-white p-4">
        ...
        </div> : <div className="mt-4 text-xs text-slate-700"><ol className="list-decimal space-y-2 pl-5"><li>Разместите read-only коннектор из каталога <code>integrations/bitrix-pull</code> на сайте.</li><li>Создайте случайный токен, сохраните его SHA-256 на сайте.</li><li>Нажмите «Подключить 1С-Битрикс Pull API» и укажите HTTPS URL и исходный токен.</li><li>Загрузите формы, выберите формы с телефоном и запустите синхронизацию.</li></ol><div className="mt-3 rounded-xl border border-violet-200 bg-violet-50 p-3">Pull API только читает результаты форм и не изменяет сайт или CRM.</div></div>}
      </div>}
