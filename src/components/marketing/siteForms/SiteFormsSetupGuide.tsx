  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-blue-200 bg-blue-50/50 shadow-sm">
      ...
      {expanded && <div className="border-t border-blue-200 bg-white p-4">
        ...
        </div> : <div className="mt-4 text-xs text-slate-700"><a href="/downloads/pbxpuls-bitrix-connector-v5.8.0.zip" download className="mb-4 inline-flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-3 font-black text-white"><Download className="h-4 w-4" />Скачать модуль для 1С-Битрикс</a><ol className="list-decimal space-y-2 pl-5"><li>Распакуйте модуль и объедините каталог <code>local/</code> с каталогом <code>local/</code> сайта.</li><li>Создайте серверный конфигурационный файл по шаблону из архива.</li><li>Создайте случайный токен, сохраните на сайте только его SHA-256.</li><li>Нажмите «Подключить 1С-Битрикс Pull API» и укажите HTTPS URL и исходный токен.</li><li>Загрузите формы, выберите формы с телефоном и запустите синхронизацию.</li></ol><div className="mt-3 rounded-xl border border-violet-200 bg-violet-50 p-3">Pull API только читает результаты форм и не изменяет сайт или CRM.</div></div>}
      </div>}
