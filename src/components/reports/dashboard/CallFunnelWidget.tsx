  ...
  return (
    <div className="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div>
        ...
        <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">Путь входящего звонка от поступления до закрытия</p>
      </div>
      <div className="mt-5 space-y-3">
        {stages.map(stage => {
          const share = percent(stage.value, max);
          return (
            <div key={stage.label} className="rounded-xl border border-slate-100 bg-slate-50/60 p-3 dark:border-slate-800 dark:bg-slate-950/20">
              <div className="flex items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-2">
                  <span className={['h-2.5 w-2.5 shrink-0 rounded-full', stage.color].join(' ')} />
                  <span className="truncate text-xs font-black text-slate-700 dark:text-slate-200">{stage.label}</span>
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  <span className="font-mono text-sm font-black text-slate-950 dark:text-white">{stage.value.toLocaleString('ru-RU')}</span>
                  <span className={['rounded-full px-2 py-1 text-[10px] font-black', stage.soft].join(' ')}>{share}%</span>
                </div>
              </div>
              <div className="mt-3 h-2 overflow-hidden rounded-full bg-white ring-1 ring-slate-200/60 dark:bg-slate-800 dark:ring-slate-700">
      ...
      {!inbound && <div className="mt-4 rounded-xl border border-dashed border-slate-200 bg-slate-50 p-3 text-center text-xs font-semibold text-slate-400 dark:border-slate-800 dark:bg-slate-950/40">Нет входящих звонков за выбранный период</div>}
    </div>
  );
}
