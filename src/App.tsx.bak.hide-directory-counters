...
const MiniRubleIcon = ({ className = "h-3.5 w-3.5" }: { className?: string }) => (
  <svg viewBox="0 0 24 24" className={className} fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    <path d="M7 5h7.2a4.2 4.2 0 0 1 0 8.4H7" />
    <path d="M7 13.4h9" />
    <path d="M7 17h9" />
    <path d="M7 5v14" />
  </svg>
...
const StatusSvg = ({ type, className = "h-3.5 w-3.5" }: { type: 'client' | 'internal' | 'supplier' | 'government'; className?: string }) => {
...
const DirectoryStatusIcon = ({ entry }: { entry: DirectoryEntry }) => {
    ...
    return (
      <div className="space-y-6 animate-fade-in font-sans pb-10">
        ...
        <div className="bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-[#334155] rounded-xl shadow-xs overflow-hidden">
          ...
          <div className={(monitorMode === "calls" ? "block " : "hidden ") + "overflow-x-auto"}>
            <table className="w-full text-left font-sans text-xs">
              <thead className="bg-slate-50 dark:bg-[#172132] border-b border-slate-200 dark:border-slate-800 text-[10.5px] font-bold text-slate-500 dark:text-slate-400 uppercase select-none">
                <tr>
                  <th className="py-2 px-3">НАПРАВЛЕНИЕ / ДЕПАРТАМЕНТ</th>
                  ...
                  <th className="py-2 px-3">% ПРОПУЩЕННЫХ (SLA)</th>
                </tr>
              </thead>
