...
## Remote SIP Phonebooks

PBXPuls can publish the directory as vendor-specific XML for Grandstream and
Yealink phones.

- profile management is implemented in `server/phonebookGateway.ts`;
- public device URLs use `/phonebook/{format}/{slug}.xml`;
- every profile has independent HTTP Basic credentials and may be disabled;
- secrets are generated by the server, stored only as hashes and returned once;
- shared profiles contain only shared contacts;
- owner-scoped profiles combine shared contacts with that owner's private contacts;
- spam and blacklisted contacts are excluded;
...
- Producers never call Telegram. They submit normalized events to `NotificationEventService` with event type, category, severity, entity, stable dedupe key, active/resolved status, occurrence time and safe metadata.
...
- Active first-release producers are low balance/recovery, delayed missed-without-callback, confirmed trunk down/recovery, database outage/recovery and new critical `system_events`. Catalog-only types do not produce synthetic events.
