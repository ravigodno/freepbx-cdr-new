...
export class VoiceGatewayService {
  ...
  async handleRawEvent(raw:any,options:{tenantId?:number;synthetic:boolean;bindingId?:number;traceId?:string}){
    ...
    if(event.eventType==='StasisStart'){
      ...
      enforceVoiceBindingPolicy(binding,options.synthetic);if(!options.synthetic&&binding.matchType==='controlled_test_extension')await this.liveGuard?.({tenantId,binding,event,raw,traceId});const agent=await this.agents.resolve(binding,options.synthetic);
      ...
      await this.store.query(`INSERT INTO ai_conversation_messages(tenant_id,conversation_id,sequence_no,role,content,content_json)VALUES(?,?,1,'system','voice_session_started',?)`,[tenantId,conversationId,JSON.stringify({voiceSessionId:session.id})]);await this.sessions.transition(tenantId,session.id,'entering_stasis',traceId);const active=await this.sessions.transition(tenantId,session.id,'active',traceId);this.metrics.sessionsCreated++;if(!options.synthetic&&binding.matchType==='controlled_test_extension')await this.liveStarter?.({tenantId,binding,session:active,event,traceId});return{ignored:false,session:active};
    }
