...
export class VoiceGatewayService {
  ...
  private liveStarter:((input:{tenantId:number;binding:any;session:any;event:any;raw:any;traceId:string})=>Promise<void>)|null=null;
  private handoffReturner:((input:{tenantId:number;session:any;event:any;raw:any;traceId:string;token:string;dialStatus:string})=>Promise<void>)|null=null;
  private handoffCompleter:((input:{tenantId:number;session:any;event:any;raw:any;traceId:string;token:string;dialStatus:string})=>Promise<void>)|null=null;
  setMediaCloser(closer:(tenantId:number,voiceSessionId:number,traceId:string)=>Promise<unknown>){this.mediaCloser=closer}
  setLiveHooks(hooks:{guard:(input:any)=>Promise<void>;start:(input:any)=>Promise<void>;handoffReturn?:(input:any)=>Promise<void>;handoffComplete?:(input:any)=>Promise<void>}){this.liveGuard=hooks.guard;this.liveStarter=hooks.start;this.handoffReturner=hooks.handoffReturn||null;this.handoffCompleter=hooks.handoffComplete||null}
  async handleRawEvent(raw:any,options:{tenantId?:number;synthetic:boolean;bindingId?:number;traceId?:string}){
    ...
    void this.audit.append({tenantId,traceId,actorType:'service',eventType:'voice_event_received',entityType:'voice_event',entityId:event.channelRefHash.slice(0,32),decision:'received',details:{eventType:event.eventType,channelRefHash:event.channelRefHash,bridgeRefHash:event.bridgeRefHash,synthetic:options.synthetic}}).catch(()=>{});
    if(event.eventType==='StasisStart'){
      const existing=(await this.repo.findActiveByChannel(tenantId,event.channelRefHash))[0];
      const handoffArg=String(raw?.args?.[0]||raw?.applicationArgs?.[0]||'');
      const handoffMatch=/^handoff_return:(handoff-[a-f0-9]{12}):([A-Z0-9_]+)$/.exec(handoffArg);
      const completeMatch=/^handoff_complete:(handoff-[a-f0-9]{12}):(ANSWER)$/.exec(handoffArg);
      if(existing&&completeMatch&&existing.state==='transferring'&&this.handoffCompleter){
        const session=await this.sessions.get(tenantId,Number(existing.id));
        await this.handoffCompleter({tenantId,session,event,raw,traceId,token:completeMatch[1],dialStatus:completeMatch[2]});
        return{ignored:false,reason:'handoff_complete',session:await this.sessions.get(tenantId,Number(existing.id))};
      }
      if(existing&&handoffMatch&&existing.state==='transferring'&&this.handoffReturner){
        const session=await this.sessions.get(tenantId,Number(existing.id));
        await this.handoffReturner({tenantId,session,event,raw,traceId,token:handoffMatch[1],dialStatus:handoffMatch[2]});
        return{ignored:false,reason:'handoff_return',session:await this.sessions.get(tenantId,Number(existing.id))};
      }
      ...
      void this.store.query(`INSERT INTO ai_conversation_messages(tenant_id,conversation_id,sequence_no,role,content,content_json)VALUES(?,?,1,'system','voice_session_started',?)`,[tenantId,conversationId,JSON.stringify({voiceSessionId:session.id})]).catch(()=>{});await this.sessions.transition(tenantId,session.id,'entering_stasis',traceId);const active=await this.sessions.transition(tenantId,session.id,'active',traceId);this.metrics.sessionsCreated++;if(!options.synthetic&&['controlled_test_extension','ai_extension'].includes(binding.matchType))await this.liveStarter?.({tenantId,binding,session:active,event,raw,traceId});return{ignored:false,session:active};
    }
    const row=(await this.repo.findActiveByChannel(tenantId,event.channelRefHash))[0];if(!row){this.metrics.ignoredEvents++;return{ignored:true,reason:'session_not_found'}}
    if(event.eventType==='StasisEnd'&&row.state==='transferring')return{ignored:true,reason:'handoff_dialplan_continue',session:await this.sessions.get(tenantId,Number(row.id))};
    if(event.eventType==='StasisEnd'||event.eventType==='ChannelDestroyed')return this.complete(tenantId,Number(row.id),traceId,Number(row.conversation_id),'caller_hangup');
