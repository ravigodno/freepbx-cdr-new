...
type Runtime = { tenantId: number; adapter: SyntheticMediaAdapter; aborter: AbortController; jitter: JitterBuffer; ingress: BackpressureController<AudioFrame>; egress: BackpressureController<AudioFrame>; vad: VadDetector; barge: BargeInController; metrics: MediaMetrics; unsubscribe: () => void; ingressSubscribers:Set<(frame:AudioFrame)=>void|Promise<void>>;started: number; events: number; vadState: string; overloadReported: boolean };
...
export class MediaSessionService {
  ...
  private async ingest(tenantId: number, id: number, frame: AudioFrame, traceId: string) {
    ...
    for (const item of ready) {
      ...
      } else if (event.type === 'speech_ended' && previous === 'speech') { runtime.metrics.vadTransitions++; await this.audit.append({ tenantId, traceId, actorType: 'service', eventType: 'vad_speech_ended', entityType: 'voice_media_session', entityId: String(id), decision: 'detected', details: {} }); }
    }
    runtime.ingress.clear(); await this.persist(tenantId, id, runtime);
  }
  async fixture(tenantId: number, id: number, type: SyntheticFixture, count: number, traceId: string) { const runtime = this.runtimes.get(id); if (!runtime || runtime.tenantId !== tenantId) throw new MediaError('not_found', 404, 'Active synthetic media session not found'); await runtime.adapter.fixture(type, count); return this.get(tenantId, id); }
  async injectRealtimeFixture(tenantId:number,id:number,fixture:'silence'|'speech'|'question'|'transfer_request'|'callback_request'|'tool_query',traceId:string){const runtime=this.runtimes.get(id);if(!runtime||runtime.tenantId!==tenantId)throw new MediaError('not_found',404,'Active synthetic media session not found');const samples=320,pcm=new Int16Array(samples),voiceSessionId=Number((await this.row(tenantId,id)).voice_session_id),baseSequence=runtime.events+1;if(fixture!=='silence')for(let i=0;i<samples;i++)pcm[i]=Math.round(4000*Math.sin(2*Math.PI*220*i/16000));for(let offset=0;offset<4;offset++){const frame:AudioFrame={sequence:baseSequence+offset,timestampMs:Date.now()+offset*20,direction:'ingress',codec:'slin16',sampleRate:16000,channels:1,durationMs:20,payload:new Uint8Array(pcm.buffer.slice(0)),source:fixture,traceId,voiceSessionId,mediaSessionId:id};await this.ingest(tenantId,id,frame,traceId)}return this.get(tenantId,id)}
  ...
  async enqueueEgress(tenantId:number,id:number,frame:AudioFrame){const runtime=this.runtimes.get(id);if(!runtime||runtime.tenantId!==tenantId)throw new MediaError('not_found',404,'Active media session not found');const accepted=runtime.egress.push({...frame,direction:'egress',mediaSessionId:id});runtime.metrics.record(frame.payload.byteLength,'egress',frame.timestampMs);if(!accepted&&!runtime.overloadReported){runtime.overloadReported=true;await this.audit.append({tenantId,traceId:frame.traceId,actorType:'service',eventType:'media_overloaded',entityType:'voice_media_session',entityId:String(id),decision:'dropping',details:{queue:'egress'}})}await this.persist(tenantId,id,runtime);return accepted}
