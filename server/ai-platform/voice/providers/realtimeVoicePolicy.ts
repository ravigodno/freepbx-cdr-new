...
export function composeRealtimeInstructions(context:any,language:string){
  const agent=context?.agent||{},
    ...
    persona=[
      ...
      "Ответь одним законченным предложением, обычно из 8–16 слов; допускается максимум два коротких предложения и 30 слов. Затем сразу замолчи и слушай. Уточняющий вопрос задавай только при необходимости. Не повторяй запрос, не перечисляй возможности, не объясняй ход действий и не добавляй вводные или финальные фразы.",
