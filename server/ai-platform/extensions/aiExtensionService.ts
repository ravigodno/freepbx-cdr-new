...
export class AiExtensionService{
  ...
  async validate(tenantId:number,input:any,ignoreId?:number){
    ...
    let provider="openai_realtime",voice="marin";try{const profile=JSON.parse(agent.config_json||"{}").voiceProfile||{};provider=String(profile.provider||provider);voice=String(profile.voiceId||(provider==="yandex_speechkit"?"marina":"marin"))}catch{}
    return{value,inspection,agent:{...agent,provider,voice,config_json:undefined},conflicts,ready:conflicts.length===0};
  }
  async previewCreate(tenantId:number,input:any,actor:any){
    ...
    if(!binding){
      const created:any=await this.store.query(`INSERT INTO ai_voice_route_bindings(tenant_id,binding_key,name,status,match_type,match_value_hash,safe_match_label,agent_id,agent_version_id,language,priority,dry_run_only,created_by,route_mode,provider_key,voice_key,activation_state)
        VALUES(?,?,?,'disabled','ai_extension',?,?,?,?,'ru',100,0,?,'production',?,?, 'draft')`,[tenantId,`ai_extension_${v.extension}`,v.displayName,voiceHash(tenantId,v.extension),v.extension,v.agentId,checked.agent.current_version_id,actor.actorId,checked.agent.provider,checked.agent.voice]);
  ...
  async apply(tenantId:number,previewId:number,confirm:boolean,actor:any){
    ...
    try{
      ...
      await this.store.query("UPDATE ai_voice_route_bindings SET status='active',activation_state='active',route_mode='production',dry_run_only=0,match_type='ai_extension',agent_version_id=?,provider_key=?,voice_key=?,updated_at=NOW() WHERE tenant_id=? AND id=?",[preview.agent.versionId,preview.agent.provider,preview.agent.voice,tenantId,extension.route_binding_id]);
      await this.store.query("UPDATE ai_extensions SET status='active',enabled=1,published_agent_version_id=?,provider=?,freepbx_miscapp_id=?,freepbx_custom_destination_id=?,last_synced_at=NOW(),sync_status='synced',sync_error_code=NULL,updated_at=NOW() WHERE tenant_id=? AND id=?",[preview.agent.versionId,preview.agent.provider,applied.miscApplicationId,applied.customDestinationId,tenantId,extension.id]);
