...
export class AiExtensionService{
  ...
  async previewCreate(tenantId:number,input:any,actor:any){
    ...
    const reusable=(await this.store.query("SELECT id,route_binding_id FROM ai_extensions WHERE tenant_id=? AND agent_id=? AND extension=? AND status IN('preview_ready','disabled','conflict','sync_failed') LIMIT 1",[tenantId,candidate.agentId,candidate.extension]))[0];
  ...
  async apply(tenantId:number,previewId:number,confirm:boolean,actor:any){
    ...
    try{
      ...
      await this.store.query("UPDATE ai_extensions SET status='applying',enabled=0,sync_status='applying',sync_error_code=NULL,updated_at=NOW() WHERE tenant_id=? AND id=?",[tenantId,extension.id]);
