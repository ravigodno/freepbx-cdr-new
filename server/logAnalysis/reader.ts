...
export interface DirectLogSearchOptions { limit?:number; maxBytes?:number; from?:string; to?:string; severity?:string; search?:string; ip?:string; extension?:string; phone?:string; callId?:string; uniqueid?:string; linkedid?:string; identifiers?:string[]; onlyErrors?:boolean; includeRotated?:boolean; includeCurrent?:boolean; candidatePaths?:string[]; signal?:AbortSignal }
export async function searchAllowedLogSource(sourceKey:string,options:DirectLogSearchOptions={}){const source=resolveLogSource(sourceKey);if(!source?.canonicalPath||!['file','pm2'].includes(source.sourceType))throw new Error('Источник не входит в allowlist');const real=await fs.promises.realpath(source.canonicalPath);if(real!==source.canonicalPath)throw new Error('Некорректный canonical path');const limit=Math.max(1,Math.min(Number(options.limit)||200,500)),maxBytes=Math.max(4096,Math.min(Number(options.maxBytes)||1024*1024,2*1024*1024));const paths:string[]=options.includeCurrent===false?[]:[real],dir=path.dirname(real);if(options.includeRotated!==false){if(options.candidatePaths){for(const item of options.candidatePaths.slice(0,3)){const candidate=await fs.promises.realpath(item);if(path.dirname(candidate)!==dir||!isApprovedCandidate(candidate,real))throw new Error('Rotated path не входит в allowlist');if(!paths.includes(candidate))paths.push(candidate)}}else{const base=path.basename(real);for(const name of (await fs.promises.readdir(dir)).sort().reverse()){if(name===base||!name.startsWith(base)||!ROTATED_SUFFIX.test(name)||name.endsWith('.gz'))continue;const candidate=await fs.promises.realpath(path.join(dir,name));if(path.dirname(candidate)===dir)paths.push(candidate);if(paths.length>=14)break;}}}if(!paths.length)return{rows:[],total:0,limit,bytesRead:0,truncated:false,filesScanned:0};
 const perFile=Math.max(16384,Math.min(MAX_CHUNK_BYTES,Math.floor(maxBytes/Math.max(1,paths.length))));let bytesRead=0;const rows:any[]=[];const from=options.from?Date.parse(options.from):NaN,to=options.to?Date.parse(options.to):NaN,identifiers=(options.identifiers||[]).filter(Boolean).slice(0,12);for(const file of paths){if(bytesRead>=maxBytes||rows.length>=limit)break;const chunk=await readTail(file,Math.min(perFile,maxBytes-bytesRead),options.signal);bytesRead+=chunk.bytesRead;for(const line of chunk.lines.reverse()){const event=parseLogLine(line,source);const time=Date.parse(event.occurredAt);if(Number.isFinite(from)&&time<from||Number.isFinite(to)&&time>to)continue;if(options.severity&&event.severity!==options.severity)continue;if(options.onlyErrors&&!['critical','error'].includes(event.severity))continue;if(options.search&&!event.rawMessage.toLowerCase().includes(options.search.toLowerCase()))continue;if(options.ip&&event.ip!==options.ip)continue;if(options.extension&&event.extension!==options.extension&&!event.rawMessage.includes(options.extension))continue;if(options.phone&&event.phone!==options.phone&&!event.rawMessage.includes(options.phone))continue;if(options.callId&&event.callId!==options.callId&&!event.rawMessage.includes(options.callId))continue;if(options.uniqueid&&event.uniqueid!==options.uniqueid&&event.linkedid!==options.uniqueid&&!event.rawMessage.includes(options.uniqueid))continue;if(identifiers.length&&!identifiers.some(id=>event.uniqueid===id||event.linkedid===id||event.callId===id||event.channel===id||event.rawMessage.includes(id)))continue;rows.push({...event,eventId:undefined,originPath:path.basename(file),rotated:file!==real});if(rows.length>=limit)break;}}
 return{rows,total:rows.length,limit,bytesRead,truncated:bytesRead>=maxBytes||rows.length>=limit,filesScanned:paths.length};}

function isApprovedCandidate(candidate:string,current:string){const base=path.basename(current);return path.basename(candidate).startsWith(base)&&ROTATED_SUFFIX.test(path.basename(candidate))&&!candidate.endsWith('.gz');}
