...
export function buildSecurityChecks(input: { firewall: any; fail2ban: any; ports: SecurityListeningPort[]; services: any[] }): SecurityCheckResult[] {
  ...
  for (const [port, title, severity] of [[3306,'MariaDB на wildcard','high'],[5038,'AMI на wildcard','high'],[3000,'PBXPuls на wildcard','medium'],[5060,'SIP на wildcard','medium']] as const) {
    ...
    add({ id:`port_${port}_wildcard`, group:'ports', title, status: socket ? 'warning' : 'passed', severity: socket ? severity : 'info', summary: socket ? `Слушает на всех интерфейсах — требуется проверка Firewall (${socket.protocol.toUpperCase()} ${socket.address}:${port})` : 'Wildcard listener не обнаружен', recommendation: socket ? 'Проверьте bind-адрес и разрешающие правила Firewall.' : undefined, evidence: socket ? { address: socket.address, protocol: socket.protocol, exposure: socket.exposure } : undefined });
  }
  const asterisk = input.services.find(item => item.name === 'asterisk');
  add({ id:'asterisk_service', group:'services', title:'Служба Asterisk', status: !asterisk||asterisk.active==='unknown' ? 'unknown' : asterisk.active === 'active' ? 'passed' : 'failed', severity: asterisk?.active === 'inactive' ? 'critical' : 'info', summary: asterisk ? `Состояние: ${asterisk.active}; метод: ${asterisk.detectionMethod||'unknown'}` : 'Служба не обнаружена',evidence:asterisk?.evidence });
  return result;
}
