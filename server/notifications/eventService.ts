...
export class NotificationEventService {
  ...
  private async createEvent(event:NotificationEventInput,state:any,ruleEventType=event.eventType):Promise<number>{
    ...
    const channels=await queryPBXPulsDb("SELECT channel,enabled FROM notification_channels WHERE channel IN('telegram','bitrix24') ORDER BY channel");
    ...
    for(const channel of channels){const channelId=String(channel.channel),decision=evaluateDelivery(event,{global,channelEnabled:bool(channel.enabled),categoryEnabled:global.categories[event.category]!==false,rule,lastNotifiedAt:state?.last_notified_at?new Date(state.last_notified_at):null,duplicate:duplicateRows.length>0,now:new Date()});
      await queryPBXPulsDb(`INSERT INTO notification_deliveries(event_id,channel_id,channel,event_key,status,attempt_count,last_error,next_attempt_at,payload_json,attempted_at,created_at)VALUES(?,?,?,?,?,0,?,IF(?='pending',UTC_TIMESTAMP(),NULL),?,UTC_TIMESTAMP(),UTC_TIMESTAMP())`,[eventId,channelId,channelId,event.dedupeKey,decision.status,decision.status==='pending'?null:decision.reason,decision.status,JSON.stringify({reason:decision.reason})]);}
    return eventId;
  }
}
