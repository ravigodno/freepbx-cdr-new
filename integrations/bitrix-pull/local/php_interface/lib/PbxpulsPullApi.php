...
final class PbxpulsPullApi
{
    ...
    private function leads(): array
    {
        ...
        while ($row = $rows->fetch()) {
            ...
            $items[] = ['eventId' => 'bitrix-form-'.$row['FORM_ID'].'-result-'.$row['ID'], 'formId' => (string)$row['FORM_ID'],
    ...
    private function clicks(): array
    {
        ...
        while($row=$rows->fetch()){$lastId=max($lastId,(int)$row['id']);$items[]=['id'=>(string)$row['id'],'eventId'=>(string)$row['event_id'],'eventType'=>'phone_click','eventTime'=>(new DateTimeImmutable((string)$row['event_time']))->format(DateTimeInterface::ATOM),'pageUrl'=>(string)$row['page_url'],'referrer'=>(string)$row['referrer'],'phoneText'=>(string)$row['phone_text'],'phoneHref'=>(string)$row['phone_href'],'sessionId'=>(string)$row['session_id'],'utm'=>['source'=>(string)$row['utm_source'],'medium'=>(string)$row['utm_medium'],'campaign'=>(string)$row['utm_campaign'],'content'=>(string)$row['utm_content'],'term'=>(string)$row['utm_term']],'ipHash'=>(string)$row['ip_hash'],'userAgent'=>(string)$row['user_agent']];}
        return['items'=>$items,'nextCursor'=>(string)$lastId,'hasMore'=>count($items)>=$limit];
    }
